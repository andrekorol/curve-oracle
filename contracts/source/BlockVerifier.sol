...
library BlockVerifier {
    function extractStateRootAndTimestamp(bytes memory rlpBytes)
        internal
        view
        returns (
            bytes32 stateRoot,
        ...
        )
    {
        assembly {
            ...
            blockNumber := shr(
                sub(256, mul(blockNumberLength, 8)),
                mload(blockNumberPointer)
            )
            ...
            blockTimestamp := shr(
                sub(256, mul(timestampLength, 8)),
                mload(timestampPointer)
            )
        }
    }
}
