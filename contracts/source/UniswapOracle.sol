...
contract UniswapOracle {
        ...
        )
    {
        ...
        Rlp.Item[] memory accountDetails =
    ...
    {
        bytes32 storageRootHash;
        (storageRootHash, blockNumber, blockTimestamp) = getAccountStorageRoot(
            address(uniswapV2Pair),
            proofData
        );
        require(
            blockNumber <= block.number - minBlocksBack,
            "Proof does not span enough blocks"
        );
        require(
            blockNumber >= block.number - maxBlocksBack,
            "Proof spans too many blocks"
        );
        ...
        uint256 reserve0Reserve1TimestampPacked =
        ...
        reserve1 = uint112(
            (reserve0Reserve1TimestampPacked >> 112) & (2**112 - 1)
        );
    ...
    function getPriceRaw(
        ...
        uint256 historicPriceCumulativeLast;
        {
            ...
            uint256 secondsBetweenReserveUpdateAndHistoricBlock =
                historicBlockTimestamp - reserveTimestamp;
        ...
        uint256 secondsBetweenProvidedBlockAndNow =
            block.timestamp - historicBlockTimestamp;
        price =
            (getCurrentPriceCumulativeLast(
                uniswapV2Pair,
                denominationTokenIs0
            ) - historicPriceCumulativeLast) /
